...
public class TimeSlotController {
    ...
    @PostMapping("/add")
    public ResponseEntity<?> addTimeSlot(@RequestBody @Valid TimeSlot timeSlot, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity<?> updateTimeSlot(@PathVariable Integer id, @RequestBody TimeSlot timeSlot, Errors errors) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }

...
public class WorkSpaceController {
    ...
    @PostMapping("/add")
    public ResponseEntity<?> addWorkSpace(@RequestBody @Valid WorkSpace workSpace, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity<?> updateWorkSpace(@PathVariable Integer id, @RequestBody @Valid WorkSpace workSpace, Errors errors) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }

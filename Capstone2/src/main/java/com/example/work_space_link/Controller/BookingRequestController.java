...
public class BookingRequestController {
    ...
    //endpoint
    @PostMapping("/add")
    public ResponseEntity<?> addBookingRequest(@RequestBody @Valid BookingRequest bookingRequest, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity<?> updateBookingRequest(@PathVariable Integer id, @RequestBody @Valid BookingRequest bookingRequest, Errors errors) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
